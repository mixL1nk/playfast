...
def analyze_method_with_decompilation(
    class_name: str, method_name: str, bytecode: list, apk_path: str
) -> dict:
...
def analyze_webview_security_decompiled(apk_path: Path):
    ...
    if unclear_methods:
        for method in unclear_methods:
            ...
            for ev in method["evidence"]:
                ...
                print("     (Value could not be determined statically)")
    else:
        print("   None found")
    ...
    total_js_methods = (
        len(js_enabled_methods) + len(js_disabled_methods) + len(unclear_methods)
    )
    ...
    if js_enabled_methods or js_disabled_methods:
        ...
        example = (
            js_enabled_methods[0] if js_enabled_methods else js_disabled_methods[0]
        )
